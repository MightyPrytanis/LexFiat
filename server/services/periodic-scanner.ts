...
export class PeriodicScannerService {
  ...
  async performPeriodicScan(): Promise<PeriodicScanResult> {
    console.log(`🔄 Starting periodic ${this.config.scanType} scan...`);
    ...
    try {
      ...
      if (this.config.autoGenerateDocs && newComponents > 0) {
        try {
          ...
          console.log(`📚 Generated documentation for ${docsGenerated} components`);
        } catch (error) {
      ...
      console.log(`✅ Periodic scan completed in ${scanDuration}ms`);
      console.log(`   📊 New: ${newComponents}, Updated: ${updatedComponents}, Candidates: ${cypherCandidates}`);
      
      // Log significant findings
      if (newComponents > 5) {
        console.log(`🎯 Significant discovery: ${newComponents} new components found`);
      }
      
      if (cypherCandidates > 0) {
        console.log(`🚀 Found ${cypherCandidates} potential Cyrano MCP candidates`);
      }
